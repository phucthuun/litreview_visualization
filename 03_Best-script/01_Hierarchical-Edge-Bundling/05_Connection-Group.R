...
#idea 3 >> we draw the top 20% of the densest internode connections
connect.clean <- connect_OverlapOrUnique %>%
  filter(Proportion >= quantile(connect_OverlapOrUnique$Proportion, 0.95))%>%
  arrange(Proportion)
...
message('Create ', file.path(result_path, "HEB_Nodes.pdf"))
ggsave(filename = file.path(result_path, "HEB_AllEdges.pdf"), p5, width = 1228, height = 634, units = "px", dpi = "screen")
message('Create ', file.path(result_path, "HEB_AllEdges.pdf"))
ggsave(filename = file.path(result_path, "HEB_Top10Edges.pdf"), p4, width = 1228, height = 634, units = "px", dpi = "screen")
message('Create ', file.path(result_path, "HEB_Top20Edges.pdf"))