...
if (interactive() ){
  ...
  if(myPlotType == '1') {
    ...
    p<-ggplot() +
      ...
      # Labels position (centroids)
      geom_sf(data = symbol_pos, pch = 21, alpha = .6, aes(size=n.Entry, fill = n.Entry, color = n.Entry, text=text.location), show.legend = F)+
      scale_fill_gradientn(colours=magma(10),
    ...
    p
    ggplotly(p, tooltip = 'text') %>% layout(geo=list(zoom=2.5))
...
us_cities = symbol_pos %>%
  mutate(lat=str_extract(geometry),
         lon=)
fig <- us_cities 
fig <- fig %>%
  plot_ly(
